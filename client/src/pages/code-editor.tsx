...
export default function CodeEditor() {
  ...
  const isOwner = user?.userPermissions?.role === 'owner';
  const canEditCode = user?.userPermissions?.permissions?.includes('edit_code') || isOwner;
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="flex items-center gap-2">
              <Badge variant={user?.userPermissions?.role === 'owner' ? 'default' : 'secondary'}>
                Role: {user?.userPermissions?.role || 'user'}
              </Badge>
            </div>
            <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-2">
              <Badge variant={user?.userPermissions?.permissions?.includes('manage_users') ? 'default' : 'secondary'}>
                User Management: {user?.userPermissions?.permissions?.includes('manage_users') ? 'Enabled' : 'Disabled'}
              </Badge>
            </div>
          </div>
          
          {user?.userPermissions?.permissions && user.userPermissions.permissions.length > 0 && (
            <div className="mt-4">
